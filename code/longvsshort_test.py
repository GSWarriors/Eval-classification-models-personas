...
from longvsshort import longvsshort
...
def main(train_df, valid_df, test_df):
    ...
    checkpoint = torch.load("/Users/arvindpunj/Desktop/Projects/NLP lab research/Extracting-personas-for-text-generation/savedmodels/resumemodel.pt")
...
def resume_training(train_df, valid_df, longshort_params, epoch, prev_loss):
    #start from next epoch
    epoch = epoch + 1

    print()
    print("the new epoch is: " + str(epoch))
    ...
    #reload longest training and validation responses
    longest_train_responses = longshort_params.parse_long_responses(training_responses)
    longest_validation_responses = longshort_params.parse_long_responses(validation_responses)

    encoded_training_dict, smallest_convo_size = create_encoding_dict(longshort_params, longest_train_responses)
    encoded_validation_dict, smallest_convo_size = create_encoding_dict(longshort_params, longest_validation_responses)

    create_training_file(training_personas, longest_train_responses)
    create_validation_file(validation_personas, longest_validation_responses)
    print("continuing training")

    longshort_params.prev_loss = prev_loss
    longshort_params.train_model(training_personas, validation_personas, encoded_training_dict, encoded_validation_dict, epoch)
